...
> Data Lab：这个实验比较简单；Bomb Lab：这个实验比较有趣，其实类似这样的汇编语言程序很多时候都会考察递推式，递归，数据结构这几类问题；Attack Lab：这个实验比较难，需要一定基础或查阅不少资料；Architecture Lab：感觉这个实验更像是编译类的实验 (毕竟涉及到了汇编代码优化)，比想象中的要简单不少；Cache Lab：感觉是目前做的最难的实验，在学习了并行计算后对这种分块处理会有进一步的了解与体会，尤其是当自己使用 CUDA 编写矩阵转置的时候；Shell Lab：一个简单且无聊的实验；Proxy Lab：一个很不错的实验，虽然学习过计算机网络这门课程，但是在编写 Web 代理程序时感觉掌握的程度还不是很高
...
在运行 Part B 部分时的测评程序时，会出现报错：

```bash
sh: 1: valgrind: not found
```

说明没有安装 `valgrind`，运行 `sudo apt install valgrind` 安装即可。
...
对于这类矩阵问题，例如矩阵转置和矩阵相乘问题，首先考虑的是用分块技术对矩阵进行分块。由于在本部分的实验中，使用的参数是 `-s 5 -E 1 -b 5`，因此可以知道这个缓存一共有 32 组，每组中只有 1 行，可以存下 32 字节，即可以存下 8 个 `int` 类型的数据。即缓存的容量为 $32\times 8$ 个 `int` 类型数据。那么可以认为缓存可以存储矩阵的 8 行数据。每 8 行之后的数据，就会发生缓存的冲突。因此考虑分块大小为 $8\times8$：
...
```
这种分块写法其实就类似于 CUDA 程序的核函数写法。在外两层循环中，表示的是不同的矩阵块。而通过最内层两层循环便可以完成转置操作，而在 CUDA 程序中是可以通过 `threadIdx.x` 和 `threadIdx.y` 来读取。通过这种简单的分块方式，可以得到测试结果如下：

![](image/csapp-cachelab-partB-32-1.png)

测评的结果是缓存的失效次数达到了 343，这个与满分还有一定差距。说明这个简单的分块方式还有一定的优化空间。

通过上述转置的方式，不难发现当 `ii = jj` 的情况下，首先会读取 `A[ii][jj]`，此时 A 矩阵的内容会被写入缓存，而再次写入到 `B[jj][ii]` 的时候，与其它转置情况不一样的是，此时的 `ii` 与 `jj` 是相同的，那么矩阵 B 的内容就会覆盖矩阵 A 刚写入缓存中的内容。因为它们的地址索引到了相同的缓存地址当中，而其它情况下对应的缓存地址是不相同的。

为了解决这种问题，我们需要更高效的利用缓存。因此考虑将缓存的一行先通过寄存器先存储起来 (在 CUDA 程序中就是用共享内存)。因此按行存储起来之后，可以得到程序如下：
...
![](image/csapp-cachelab-partB-32-2.png)

可以看到已经达到了满分的要求。

##### 2. $64\times64$ 矩阵转置

这一部分是真的非常难，搜了不少资料才能理解如何实现。

首先测试的矩阵规模从 $32\times 32$ 变成了 $64\times 64$。原来的缓存可以存储矩阵 8 行 32 个 `int` 类型的数据，而现在的缓存只能存储矩阵 4 行 64 个 `int` 类型的数据。这就说明了，在每读取矩阵 4 行数据之后，矩阵中的数据就会再次映射到缓存中的相同地址，从而发生冲突失效。因此，如果继续使用 $8\times8$ 的分块方式，那么就会出现块内冲突，这肯定是不合法的方式。

因此，我们需要采用 $4\times4$ 的分块方式。但是，只使用 $4\times4$ 的分块方式，并不能充分利用缓存，因为一组缓存是可以存储 8 个数据的。因此，接下来仍然采用 $8\times8$ 的分块方式，但是在处理的时候，以 4 个 $4\times4$ 的分块来对其进行处理。那么普通的分块方式会是这样：

<img src="image/csapp-cachelab-partB-4x4-1.png" style="zoom: 67%;" />

但是这样的分块方式会导致一个问题：因为每间隔 4 行，缓存的行就会被覆盖，因此在写入 B' 的时候，由于 B‘ 和 A’ 相隔了 4 行，所以 A' 的缓存会被 B‘ 覆盖。因此考虑将 B' 和 C' 互换位置：

<img src="image/csapp-cachelab-partB-4x4-2.png" style="zoom:67%;" />

可以看到，B’ 的存储也变成了逆序存储。这样做的原因在于：如果数据不是逆序存放的话，则 C‘ 和 D’ 第一行的缓存会覆盖 B‘ 第一行的缓存，因此将 B’ 第一行的数据放到上图 C‘ 第一行位置的时候又会出现冲突失效。而当数据逆序存放时，尽管 C’ 和 D‘ 的缓存会覆盖 B’ 第一行的缓存，但是我们读取的是 B‘ 第四行的数据 (实际上是第一行的数据，但只是因为逆序存放了而已)，此时第四行的数据仍然在缓存中，这样就不会出现冲突失效了。

为了能够按行的方式将数据存放的目的地，需要通过按列的方式读取 C 和 D 分块中的数据。首先对于 B‘ 来说，从其第四行开始替换回其目标地址，在处理完后，为了能够补上这个位置，将 C 分块中的数据按列的方式读取并存入到这里，从而刚好可以衔接上。

大致的过程示意图如下：

<center class="half">
    <img src="image/csapp-cachelab-partB-4x4-3.png" width="250"/>
	<img src="image/csapp-cachelab-partB-4x4-4.png" width="250"/>
</center>

<center class="half">
    <img src="image/csapp-cachelab-partB-4x4-5.png" width="250"/>
	<img src="image/csapp-cachelab-partB-4x4-6.png" width="250"/>
</center>
...
![](image/csapp-cachelab-partB-64.png)

最后的结果也是达到了满分的要求

##### 3. $67\times 61$ 矩阵转置

由于测试矩阵的规模是不规则的，因此可以采用分治的方法，分为规则部分和不规则部分分开进行处理。继续采用 $8\times8$ 的分块方式，那么 $67\times61$ 的分块可以被划分成规模大小分别为 $64\times 56$ 的矩阵，$64\times5$ 的矩阵和 $3\times 61$ 的矩阵。其中 $64\times56$ 的矩阵可以按第一部分的方式进行处理。而其余不规则的矩阵可以根据其特征，取相应数量的寄存器进行存储达到要求。
...
![](image/csapp-cachelab-partB-67x61.png)

也达到了满分的要求，毕竟 $67\times61$ 测试样例的要求比较宽松。

最后整个缓存实验的总测评结果如下：

![](image/csapp-cachelab-partB.png)
